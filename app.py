...
def ensure_data_files():
    """Download missing data files from Google Drive once."""
    missing = [name for name in DRIVE_FILES if not os.path.exists(name)]
    if missing:
        progress = st.progress(0, text="Downloading data files...")
        for i, name in enumerate(missing):
            progress.progress((i) / len(missing), text=f"Downloading {name}...")
            try:
                download_from_drive(DRIVE_FILES[name], name)
