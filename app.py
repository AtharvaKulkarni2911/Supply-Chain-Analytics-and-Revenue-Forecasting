...
def ensure_data_files():
    ...
    missing = [name for name in DRIVE_FILES if not os.path.exists(name)]
    if not missing:
        return

    progress = st.progress(0, text="Downloading data files from Google Drive...")
...
def load_all_data():
    ...
    if master.empty and not orders.empty and not items.empty:
        ...
        if "customer_id" not in orders.columns:
            for alt in ["customerid", "customer"]:
                if alt in orders.columns:
                    orders = orders.rename(columns={alt: "customer_id"})
                    break

        # items: order_id  (Olist raw dataset uses order_id directly)
        if "order_id" not in items.columns:
            for alt in ["orderid", "order"]:
                if alt in items.columns:
                    items = items.rename(columns={alt: "order_id"})
                    break

        # items: freight_value → shipping_charges alias
        ...
        if not customers.empty and "customer_id" not in customers.columns:
            for alt in ["customerid", "customer"]:
                if alt in customers.columns:
                    customers = customers.rename(columns={alt: "customer_id"})
                    break

        # payments: order_id
        if not payments.empty and "order_id" not in payments.columns:
            for alt in ["orderid", "order"]:
                if alt in payments.columns:
                    payments = payments.rename(columns={alt: "order_id"})
                    break

        # products: product_id
        if not products.empty and "product_id" not in products.columns:
            for alt in ["productid", "product"]:
                if alt in products.columns:
                    products = products.rename(columns={alt: "product_id"})
                    break
        ...
        # freight_value → shipping_charges alias on master (post-merge)
