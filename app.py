...
# ─────────────────────────────────────────────
DRIVE_FILES = {
    # master_dataset.csv is downloaded FIRST — if present, all 5 component
    # files are skipped entirely, making startup much faster.
...
def ensure_data_files():
    ...
    # Download master_dataset.csv first — if it succeeds, skip all 5 component files
    if not os.path.exists("master_dataset.csv"):
        progress = st.progress(0, text="Downloading master_dataset.csv from Google Drive...")
        try:
            gdown.download(
                f"https://drive.google.com/uc?id={DRIVE_FILES['master_dataset.csv']}",
                "master_dataset.csv",
                quiet=True,
                fuzzy=True,
            )
    ...
    # If master exists now, no need for component files
    if os.path.exists("master_dataset.csv"):
        return

    # Fall back: download the 5 component files
    ...
    for i, name in enumerate(missing):
        ...
        try:
            gdown.download(
                f"https://drive.google.com/uc?id={DRIVE_FILES[name]}",
                name,
                quiet=True,
                fuzzy=True,
            )
...
def load_all_data():
    ...
    def norm_col(df, target, alts):
        """Rename first matching alt-name to target if target not already present."""
    ...
    # ── Load master first — fast path ───────────────────────────
    master = try_load(["master_dataset.csv"])
    if not master.empty:
        master = parse_dates(master)
        # Ensure derived columns exist on pre-built master
        if "freight_value" in master.columns and "shipping_charges" not in master.columns:
            master["shipping_charges"] = master["freight_value"]
        if "price" in master.columns and "shipping_charges" in master.columns and "item_revenue" not in master.columns:
            master["item_revenue"] = master["price"] + master["shipping_charges"]
        for col in ["order_year","order_month","order_quarter","order_day_of_week"]:
            if col not in master.columns and "order_purchase_timestamp" in master.columns:
        ...
        # Return stubs for component dataframes (not needed when master exists)
        orders    = master[["order_id","order_status","order_purchase_timestamp","customer_id"]].drop_duplicates("order_id") if "order_id" in master.columns else pd.DataFrame()
        items     = pd.DataFrame()
    ...
    # ── Slow path: build master from component files ─────────────
    ...
    # ── Merge — keep only necessary columns from each side to avoid
    #    duplicate column names that would break subsequent merges ─
    master = orders.copy()

    if not customers.empty and "customer_id" in master.columns and "customer_id" in customers.columns:
        # Drop any columns in customers that already exist in master (except the key)
        cust_cols = ["customer_id"] + [c for c in customers.columns if c not in master.columns]
        master = master.merge(customers[cust_cols], on="customer_id", how="left")

    if not items.empty and "order_id" in master.columns and "order_id" in items.columns:
        item_cols = ["order_id"] + [c for c in items.columns if c not in master.columns]
        master = master.merge(items[item_cols], on="order_id", how="left")

    if not products.empty and "product_id" in master.columns and "product_id" in products.columns:
        prod_cols = ["product_id"] + [c for c in products.columns if c not in master.columns]
        master = master.merge(products[prod_cols], on="product_id", how="left")

    if not pay_agg.empty and "order_id" in master.columns:
        pay_cols = ["order_id"] + [c for c in pay_agg.columns if c not in master.columns]
        master = master.merge(pay_agg[pay_cols], on="order_id", how="left")

    # Re-parse dates post-merge
    master = parse_dates(master)
    ...
    if "order_delivered_timestamp" in master.columns and "order_purchase_timestamp" in master.columns:
        ...
        if "order_estimated_delivery_date" in master.columns:
            master["on_time_delivery"] = (
                master["order_delivered_timestamp"] <= master["order_estimated_delivery_date"]
            ).astype("Int64")

    if "order_approved_at" in master.columns and "order_purchase_timestamp" in master.columns:
    ...
    if "shipping_charges" not in master.columns and "freight_value" in master.columns:
        master["shipping_charges"] = master["freight_value"]
    ...
    if monthly.empty and not master.empty:
        if "order_status" in master.columns:
            tmp = master[master["order_status"] == "delivered"].copy()
        else:
            tmp = master.copy()
        if all(c in tmp.columns for c in ["order_year","order_month","item_revenue"]):
